...
let box = 0; // check if board is full
...
function loadDOM(){
    createBoard();
    player.innerHTML = "Current Player is " + currentPlayer;
...
function clickBox(){
    ...
    // this.dataset.id returns a string. we must convert to an int.

    // 1. check if the element is within the gameboard.
    // 2. check if the element below it is taken. 
    // 3. check if the square we selected is not taken
    ...
    if (click < fullBoard 
        ...
        if (box === fullBoard){
            setTimeout(() => alert("All spaces filled, restart game", 300))
